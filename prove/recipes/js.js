const recipes = [
	{
		...
		image: './assets/sweet-potato-waffle-md.jpg',
		recipeIngredient: [
			'2 separated eggs',
			'1/4 C Oil',
			'1/4 tsp salt',
	...
	},
	{
		...
		image: './assets/escalopes-de-poulet-a-la-creme.webp',
		recipeIngredient: [
	...
	},
	{
		...
		image: './assets/roasted-potatoes.webp',
		recipeIngredient: [
			'3-4 Medium Potatoes',
			'1 Tbsp Olive oil',
	...
	},
	{
		...
		image: './assets/black-beans-and-rice.jpg',
		recipeIngredient: [
	...
	},
	{
		...
		image: './assets/chicken-curry.webp',
		recipeIngredient: [
			'4 Slices Bacon',
			'1 clove Garlic',
			'2 Tbsp Flour',
	...
	},
	{
		...
		image: './assets/chocolate-chip-cookies.jpg',
		recipeIngredient: [
	...
	},
	{
		...
		image: './assets/german-gooseberry-cake.jpg',
		recipeIngredient: [
			'For the Cake Base:',
...
{
    document.querySelector('main').innerHTML = "<p>No recipes found</p>";
}

function displayRecipe(recipe, first)
{
    ...
    {
        container.innerHTML += '<div class="buffer">'
    }
    let newHTML = `
    <div class="recipe"><img src=${recipe.image} alt="${recipe.name} dish">
    <div class="container">`;
    for (let i = 0; i < recipe.tags.length; i++)
    {
        newHTML += `<button>${recipe.tags[i]}</button>`;
    }
    ...
    {
        newHTML += '<span aria-hidden="true" class="icon-star">⭐</span>';
        console.log('star');
    }
    for (let i = recipe.rating; i < 5; i++)
    {
        newHTML += '<span aria-hidden="true" class="icon-star-empty">☆</span>';
        console.log('no star');
    }
    newHTML += '</span></div></div>';
...
function search()
{
    ...
    sortedRecipes.sort(sortRecipesByName);

    let first = true;
    for (let i = 0; i < sortedRecipes.length; i++)
    {
        displayRecipe(sortedRecipes[i], first);
        first = false;
    }
}
...
clearRecipes();
displayRecipe(recipes[Math.floor(Math.random() * recipes.length)], true);