...
{
    calls++;
    console.log("Call #" + calls + "| A: " + a.name + ", B: " + b.name)
    if (a.name === b.name)
        return 0;
    if (a.name > b.name)
        return 1;
...
const people = [{name:'James'}, {name:'Bob'}, {name:'Josh'}, {name:'Aaron'}, {name:'Ryan'}, {name:'Samus'}];
people.sort(sortPeopleByName); // pass a function that .sort() can comprehend
console.log(people);
